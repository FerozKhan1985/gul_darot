...
# Set page config
st.set_page_config(page_title="Shahbaz Gul Darot YouTube Channel Dashboard", layout="wide")

# Helper functions
@st.cache_data
def load_data():
    data = pd.read_csv("gd_youtube_channel_data.csv")
...
# Set up input widgets
with st.sidebar:
    st.title("Shahbaz Gul Darot YouTube Channel Dashboard")
    st.header("⚙️ Settings")
    
    max_date = datetime.now().date()  # End date is today's date
    default_start_date = datetime(2024, 1, 19).date()  # Start date is 1st Jan 2024
